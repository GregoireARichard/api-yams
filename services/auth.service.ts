...
import { IAuthErrorResponse, IAuthSuccessResponse } from "../types/IAuthResponse.type";
...
export class AuthService{
    ...
    public static async signup(user: IUser): Promise<ISignUpResponse>{
        const encryptedPassword = utils.encryptPassword(user.password, 10)
        user.password = (await encryptedPassword).toString()
    ...
    public static async signin(user: IUserConnect): Promise<IAuthSuccessResponse | IAuthErrorResponse>{
